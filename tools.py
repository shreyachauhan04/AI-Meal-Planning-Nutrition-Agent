...
def lookup_ingredient_nutrition(ingredient_name: str, weight_g: float) -> str:
    ...
    return f"{weight_g}g of {ingredient_name}: {base['cals']*factor:.0f} kcal, P: {base['protein']*factor:.1f}g, C: {base['carbs']*factor:.1f}g, F: {base['fats']*factor:.1f}g"

@tool
def calculate_body_metrics(weight_kg: float, height_cm: float, age: int, gender: str, goal: str) -> str:
    """Calculates BMI and gender-specific BMR targets using the Mifflin-St Jeor equation."""
